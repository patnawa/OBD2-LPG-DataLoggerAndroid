...
- **VE map UX**: the heatmap opens on the fuel being logged (mirroring the fuel-correction map); review opens on the replayed log's side, or directly on ΔVE when both fuels are present. The map-info dialog gains a bilingual (EN/TH) "How to read the VE Map" section covering the VE physics, the three views, the confidence fading and the ΔVE color semantics.
- **VE map in session review**: the review/compare screen now rebuilds the learned VE surface from a log's `ve_map_*` columns (last row per cell wins — the logged value is the store's running mean) and shows it in a VE heatmap card with the same Petrol/LPG/ΔVE tap-cycling; multi-file compare (petrol log + LPG log) restores both sides so ΔVE works across files. Hidden for logs that predate 3.33.0. Also fixed a latent replay bug: the `Map Sample Accepted` column was never matched by the header parser (pattern said "map accepted"), so replays silently fell back to closed-loop gating instead of the store's actual accept decisions.
...
- **Corrected the SAE J1349 correction factor to the actual standard.** The method returned only the bare atmospheric factor `(99/Pd)·√(T/298)`, omitting the standard's `1.18 × cf_a − 0.18` mechanical-efficiency term. Because that term is affine with a slope above 1 it does not cancel: it understated the correction by ~18% of the deviation from unity, agreeing only at exactly standard conditions — which is where the existing test probed. At 35 °C / 100.5 kPa / 70% RH the reported factor was 1.041 against a true 1.048. The result is also now absent outside J1349's own ±7% (0.93–1.07) validity band instead of extrapolating a correction the standard does not vouch for.
- **Replaced the density-altitude approximation chain with the exact ISA inversion** `DA = 145442.16 × (1 − (ρ/ρ₀)^0.234969)`, computed from the same humid density AeroDensity already reports. The previous path stacked a pressure altitude, a virtual-temperature substitution and the "120 ft per °C" hangar rule of thumb, whose linearisation error grew with altitude and humidity — exactly where the readout matters (~130 ft high at 35 °C sea level).
- **Removed display-rounding quantisation from the density primitives.** `manifoldAirDensityKgM3` divided a rounded lbs/1000ft³ display value back down, stamping a 0.1 lbs/1000ft³ (~0.0016 kg/m³) grid onto every consumer of manifold density. kg/m³ is now the primitive and rounding happens once at the display boundary. BAD, a difference of two close densities and the value most hurt by rounding its operands first, is computed at full precision via `boostAirDensityFromKgM3`.
- **Fixed the SAE J607 reference temperature** from the borrowed ISA 288.15 K (15 °C) to its own 288.71 K (60 °F), putting it back in step with the `SAE_J607_AAD` = 76.4 lbs/1000ft³ constant derived at the correct reference.
- **Relabelled the J607 factor as "STD".** The calculation subtracts vapor pressure, which makes it Dynojet's humidity-corrected STD variant rather than classic J607 (which uses the observed barometer). The math is unchanged — it is the more physically sound of the two and matches `SAE_J607_AAD` — but the log column, sample name and gauge now say STD so a session file cannot be quoted as a J607 number it never was. The `derived_sae_j607` key is deliberately unchanged so existing logs and saved gauge layouts keep resolving.
- **Corrected the MAD and BAD gauge ranges.** BAD spans `-60…90` rather than `0…120`: it is negative under manifold vacuum, so the old floor parked the needle on the peg at idle and cruise while the top third of the scale was unreachable. MAD spans `0…160` rather than `0…80`, which pegged from roughly 130 kPa MAP upward.
- Added regression coverage for each of the above; AeroDensity suite 24/24, full `testDebugUnitTest assembleDebug` green.
