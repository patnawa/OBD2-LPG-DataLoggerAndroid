...
- **VE map on screen** — tap the new heatmap card to cycle Petrol VE / LPG VE / ΔVE, with confidence-driven opacity. It opens on the fuel being logged (like the fuel-correction map); session review opens on the side the log contains, or directly on ΔVE for a petrol+LPG compare. The learned surface now **persists across reboots** and each session feeds a **ΔVE drift alarm** (OK/WATCH/ALARM in `/api/vemap`) — the early warning for vaporizer/mixer/gas-injector aging.
- **How to read VE** — 100% = the cylinder filled exactly its displacement with manifold-density air; healthy NA engines peak ~85–95%. ΔVE (petrol − LPG) is normally a few points **positive** because gaseous fuel displaces intake air: green = normal, amber→red = the gap widening at that load (vaporizer/mixer/injector aging — service early warning), blue = inverted gap (investigate sensors/petrol baseline). Faded cells = low confidence. The full guide is in the in-app map-info dialog (EN/TH).
...
## What's New in 3.28.0 — Dashboard Readability & Air-Density Cleanup

- **Right-sized numbers** — dashboard slots now show sensible precision per metric (whole RPM/speed/temps, one decimal for voltage/boost, two for lambda) instead of forcing `.00` on everything.
- **At-a-glance warnings** — coolant, oil, intake-air temperature, and charging voltage turn amber then red as they approach their danger zones; everyday PIDs stay neutral so there are no false alarms.
- **No more frozen readings** — a slot dims when its PID stops reporting (or logging stops), so a stale value can't be mistaken for a live one, and brightens again when data resumes.
- **Air-density internals** — verified AAD/MAD physics against SAE J1349/J607 references (17/17 regression tests green) and tidied two internal spots with no change to results.

---

## What's New in 3.27.0 — Live Telemetry Graph Readability

- **Auto-scaling live graph** — the home-screen RPM/speed/boost trace now scales each channel to its own rolling peak instead of fixed ceilings, so high-revving engines no longer clip and modest boost fills the plot instead of sitting flat on the baseline.
- **Read values at a glance** — each visible series shows its current value in its own colour at the top of the graph, no tap required; the inline readouts step aside automatically when you tap to inspect a point.
- **Clearer legend toggles** — hiding a series now dims and strikes through its legend so it's obvious which channels are on.
